...
def process_pickle_file(file_path, output_dirs):
    ...
    codes_beating_surface_code, codes_not_beating_surface_code = filter_best_codes_group_by_weight(codes)

    beating_path = os.path.join(output_dirs['beating_surface_code'], os.path.basename(file_path))
    not_beating_path = os.path.join(output_dirs['NOT_beating_surface_code'], os.path.basename(file_path))
    ...
    if codes_beating_surface_code:
        ...
        logging.warning(f"NO codes found in {extract_file_and_parent_directory(file_path)} that beat the surface code.")
    if codes_not_beating_surface_code:
        save_as_pickle(not_beating_path, codes_not_beating_surface_code)
    else:
        logging.warning(f"ALL codes found in {extract_file_and_parent_directory(file_path)}  beat the surface code.")
...
def split_and_save_best_codes(base_dir):
    for root, dirs, files in os.walk(base_dir):
        ...
        for file_name in files:
            if file_name.startswith('unified_codes_with_distance_') and file_name.endswith('.pickle'):
