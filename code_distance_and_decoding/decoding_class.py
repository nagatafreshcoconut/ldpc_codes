...
class CustomDecoder:
    ...
    def __init__(self, code_configs_dir: str, code_details: Dict, decoding_details: Dict):
        """
        Args:
    ...
    def save_results_as_pickle(self, code_configs_decoded, result_subfolder):
        ...
        file_path = os.path.join(result_subfolder, f"codes_decoded_{decoder_marker}.pickle")
    ...
    def perform_decoding(self, args):
        ...
        (
            ...
            rounds,
            error_rate,
        ) = self.name_code, self.decoder, self.error_model, self.size_code, self.n_trials, self.rounds, self.error_rate
        ...
        effective_error_list, codespace_list, avg_walltime, full_filename = self.run_QEC_batch(
            input_data, rng
        )
    ...
    def perform_decoding_parallel(self, original_subfolder_name, grouped_code_configs):
        ...
        for weight, codes in grouped_code_configs.items():
            for index, code in enumerate(codes):
                ...
                try:
                    decoded_code = self.perform_decoding(args)  # Get decoding result as a dict
                ...
                grouped_code_configs[weight][index]['decoding_results'] = decoded_code['decoding_results']
                    ...
                    for filename in os.listdir(target_dir):
                        if filename.endswith(".pickle"):
                            ...
                            try:
                                ...
                                grouped_code_configs = add_hz_to_code(grouped_code_configs)
                                
                                for weight, codes in grouped_code_configs.items():
                                    codes = [code for code in codes if code['num_phys_qubits'] != code['num_log_qubits']]
                                    # grouped_code_configs[weight] = codes[2:3]  # Limiting to 2 codes for testing
                                
                                for weight, codes in grouped_code_configs.items():
                                        logging.warning(f"Weight: {weight}, Number of codes: {len(codes)}")
                                ...
                                self.perform_decoding_parallel(original_subfolder_name, grouped_code_configs)
