...
class CSSCode(StabilizerCode):
    ...
    def __init__(
        self, Hx_in,Hz_in,name,L_in=None):
        ...
        self.qubit_stab_indices_z=[np.nonzero(Hz_in_array[x,:])[0] for x in range(self.n_stab_z)]
        self.qubit_stab_indices_x=[np.nonzero(Hx_in_array[x,:])[0] for x in range(self.n_stab_x)]
