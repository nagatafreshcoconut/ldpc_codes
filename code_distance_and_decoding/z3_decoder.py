...
class Z3Decoder(BaseDecoder):
    ...
    def binary_xor_fixed_to_CNF(A,B,val,helper):
        ...
        if(val==0):
            ...
            form4="NOT("+A+")"+" v "+"NOT("+B+")"+" v "+"NOT("+helper+")"
        else:
    ...
    def binary_equal_to_CNF(A,val,helper1,helper2):
        ...
        if(val==0):
            ...
            form4="NOT("+A+")"+" v "+"NOT("+helper1+")"+" v "+"NOT("+helper2+")"
        elif(val==1):
    ...
    def z3_decoder_ini(parity_check_matrix,error_rate):
        ...
        CNF_form_hard_const=[] ##only contains constant CNF terms not involving syndrom
        ...
        for k in range(num_detectors):
            ...
            helper_vars[k] = [z3.Bool(f"helper_{k}_{i}") for i in range(len(errors_trigger_detector) - 2)]
            
            ##helper variables for normal form
            helper_CNF_vars[k]= [f"h_{k}_{i}" for i in range(len(errors_trigger_detector) - 2)]
            ...
            for i in range(1, len(errors_trigger_detector) - 2):
                constraint = z3.Xor(error_vars[errors_trigger_detector[i]], helper_vars[k][i]) == helper_vars[k][i - 1]
                opt_z3.add(z3.simplify(constraint))
                
                ##do same for CNF
                CNF_form_hard_const.append(Z3Decoder.binary_xor_to_CNF(error_CNF_vars[errors_trigger_detector[i]],helper_CNF_vars[k][i],helper_CNF_vars[k][i-1]))
                
                

            if(len(errors_trigger_detector)>2):
                constraint =z3.Xor(error_vars[errors_trigger_detector[-2]], error_vars[errors_trigger_detector[-1]]) == helper_vars[k][-1]
                opt_z3.add(z3.simplify(constraint))
                CNF_form_hard_const.append(Z3Decoder.binary_xor_to_CNF(error_CNF_vars[errors_trigger_detector[-2]],error_CNF_vars[errors_trigger_detector[-1]],helper_CNF_vars[k][-1]))
                
                
        return opt_z3,error_vars,helper_vars,errors_trigger_detector_list,CNF_form_hard_const,CNF_form_soft_const
    ...
    @staticmethod
    def z3_do_decoding(opt_z3,error_vars,helper_vars,syndrome,errors_trigger_detector_list):
        ...
        for k in range(num_detectors):
            ...
            if(len(errors_trigger_detector)==0):
                ...
                constraint = error_vars[errors_trigger_detector[0]], error_vars[errors_trigger_detector[1]]==bool_syndrom[k]
                opt_z3.add(z3.simplify(constraint))
                ...
                constraint = z3.Xor(error_vars[errors_trigger_detector[0]], helper_vars[k][0]) == bool_syndrom[k]
                opt_z3.add(z3.simplify(constraint))
    ...
    def initialize_decoders(self):
        ...
        if self.is_css:
            if(self.ignore_z==False):
                self.opt_z3_z,self.error_vars_z,self.helper_vars_z,self.errors_trigger_detector_z,self.CNF_form_hard_const_z,CNF_form_soft_const_z = Z3Decoder.z3_decoder_ini(
                    self.code.Hx,
            ...
            if(self.ignore_x==False):
                self.opt_z3_x,self.error_vars_x,self.helper_vars_x,self.errors_trigger_detector_x,self.CNF_form_hard_const_x,CNF_form_soft_const_x = Z3Decoder.z3_decoder_ini(
                    self.code.Hz,
            ...
            self.opt_z3,self.error_vars,self.helper_vars,self.errors_trigger_detector,self.CNF_form_hard_const,CNF_form_soft_const = Z3Decoder.z3_decoder_ini(
    ...
    def decode(self, syndrome: np.ndarray, **kwargs) -> np.ndarray:
        ...
        if self.is_css:
            ...
            if(self.ignore_z==False):
                # Decode Z errors
                z_correction,CNF_form_hard_flex_z=Z3Decoder.z3_do_decoding(self.opt_z3_z,self.error_vars_z,self.helper_vars_z,syndrome_x,self.errors_trigger_detector_z)
            else:
                z_correction=[0 for k in range(n_qubits)]

            if(self.ignore_x==False):
                # Decode X errors
                x_correction,CNF_form_hard_flex_x=Z3Decoder.z3_do_decoding(self.opt_z3_x,self.error_vars_x,self.helper_vars_x,syndrome_z,self.errors_trigger_detector_x)
            else:
            ...
            # Decode all errors
            correction,CNF_form_hard_flex=Z3Decoder.z3_do_decoding(self.opt_z3,self.error_vars,self.helper_vars,syndrome,self.errors_trigger_detector)
