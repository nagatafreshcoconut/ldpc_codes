...
def log_properties(code, property_name):
    print(f"Code with l={code['l']}, m={code['m']}, A={code['A_poly_sum']}, B={code['B_poly_sum']} leads to "
                ...
                if file == 'processed_consolidated_codes.pickle':
                    ...
                    try:
                        with open(file_path, 'rb') as f:
                            ...
                            for code in codes:
                                code['num_physical_qubits'] = code.pop('num_phys_qubits')
                            ...
                            log_properties(highest_values['num_logical_qubits'], 'number of logical qubits (k)')
                            log_properties(highest_values['encoding_rate'], 'encoding rate')
                            
                    except (EOFError, pickle.UnpicklingError):
                        print(f"Error reading file {file_path}. It may be corrupted or not a valid pickle file. Skipping.")
